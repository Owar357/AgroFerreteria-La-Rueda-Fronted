<template>
  ...
  >
    ...
    <div class="grid grid-cols-2 lg:grid-cols-4 gap-4 mb-6 w-full">
      ...
      >
        <div class="flex justify-between items-start mb-3">
          <p class="text-sm font-medium text-[#6d8f60]">
      ...
      >
        ...
        <p
          :class="[
            'text-3xl font-bold',
            (turnoAbierto ? diferencia : diferenciaFinal) === 0
              ? 'text-gray-500'
              : (turnoAbierto ? diferencia : diferenciaFinal) > 0
                ? 'text-green-700'
                : 'text-red-600',
        ...
        <div class="bg-white rounded-2xl border border-[#e8efe1] overflow-hidden shadow-sm">
          ...
          <div class="p-6">
            <div class="grid grid-cols-2 gap-5">
              ...
              >
                <span class="text-sm text-[#5f7b58]"
                  ><i class="pi pi-plus mr-2"></i> Otras entradas de dinero</span
                >
    ...
    <CerrarCajaDialog
      v-model:visible="cerrarCajaVisible"
      :montoEsperado="montoEsperado"
      @cerrar="finalizarCierre"
    />
  </div>
</template>
...
const calcularResumenDesdeMovimientos = (montoInicialApertura) => {
  ...
  for (const mov of todosMovimientos.value) {
    if (mov.concepto.includes('Venta contado'))
      ventasContadoSum += parseFloat(mov.monto.replace('+$', ''))
    else if (mov.concepto.includes('Venta por tarjeta'))
      ventasTarjetaSum += parseFloat(mov.monto.replace('+$', ''))
    else if (mov.concepto.includes('Transferencia recibida'))
      ventasTransferenciaSum += parseFloat(mov.monto.replace('+$', ''))
...
const showExternalMovements = () => {
  const externos = todosMovimientos.value.filter(
    (m) =>
