<template>
  ...
  >
    ...
    >
      ...
      >
        <i :class="['pi', bloqueo.icono, bloqueo.color]" style="font-size: 48px"></i>
        <h2 class="text-[22px] font-semibold text-[#1a2e1f] text-center">{{ bloqueo.titulo }}</h2>
        <p class="text-[14px] text-[#6b7280] text-center">{{ bloqueo.mensaje }}</p>
        <Button
          v-if="cajaStore.errorEstado"
          label="Reintentar"
          icon="pi pi-refresh"
          class="!bg-[#2b5e3b] !border-[#2b5e3b] text-white font-semibold px-6 py-3 rounded-lg"
          @click="cajaStore.cargarEstadoCaja()"
        />
        <Button
          v-else-if="cajaStore.estadoCargado && !cajaStore.turnoDeOtroCajero"
          label="Ir a Caja"
              ...
              <Column header="Cantidad">
                <template #body="slotProps">
                  <InputNumber
                    ...
                    @update:modelValue="posStore.recalcularSubtotal(slotProps.data)"
                    :min="1"
                    :maxFractionDigits="0"
                    showButtons
                    ...
                    <span class="text-xs font-bold text-amber-700">
                      ${{
                        (
                          ...
                          slotProps.data.descuento / slotProps.data.cantidad
                        ).toFixed(2)
                      }}
              ...
              <Column header="" style="width: 40px">
                <template #body="slotProps">
                  <Button
                    ...
                    rounded
                    size="small"
                    @click="posStore.eliminarProducto(slotProps.index)"
                  />
                </template>
              </Column>
            </DataTable>
      ...
      <div class="flex flex-col overflow-hidden" style="padding: 20px">
        ...
        <div class="flex flex-col gap-4 flex-1">
          ...
          <div class="flex gap-2 items-center">
            <InputText
              v-model="posStore.busquedaCliente"
              ...
              "
              @keyup.enter="posStore.buscarCliente"
            />
            <Button
              icon="pi pi-search"
              @click="posStore.buscarCliente"
              style="
                background-color: #ffffff;
          ...
          <div class="flex justify-between items-center">
            ...
            <span style="font-size: 13px; color: #1a2e1f"
              >${{ posStore.subtotalGravado.toFixed(2) }}</span
            >
          </div>
          <div class="flex justify-between items-center">
            <span style="font-size: 13px; color: #4b5563">Subtotal exento</span>
            <span style="font-size: 13px; color: #1a2e1f"
              >${{ posStore.subtotalExento.toFixed(2) }}</span
            >
          </div>
          <div class="flex justify-between items-center">
          ...
          <div v-if="posStore.tipoPago === 'efectivo'" class="flex flex-col gap-1.5">
            ...
            <div class="flex justify-between items-center gap-4">
              <div class="flex items-center gap-2 flex-1">
                ...
                <InputNumber
                  v-model="posStore.efectivoRecibido"
                  mode="currency"
                  currency="USD"
                  locale="en-US"
                  :min="0"
  ...
  <DialogAddCliente
    v-model="posStore.mostrarModalCliente"
...
const bloqueo = computed(() => {
  if (!cajaStore.estadoCargado) {
    return {
      ...
      color: 'text-[#6b7280]',
      titulo: 'Verificando caja…',
      mensaje: 'Un momento, se está consultando el estado de la caja.',
    }
  }
  if (cajaStore.errorEstado) {
    return {
  ...
  if (!cajaStore.cajaAbierta) {
    return {
      ...
      mensaje: 'El administrador debe aperturar la caja para poder realizar ventas.',
    }
  }
  if (cajaStore.turnoDeOtroCajero) {
    return {
      icono: 'pi-user',
      color: 'text-[#b45309]',
      titulo: 'Turno en uso',
      mensaje: `El turno está abierto por ${cajaStore.turnoActivo.cajero_nombre}. Solo ese cajero puede registrar ventas.`,
    }
  }
  return {
