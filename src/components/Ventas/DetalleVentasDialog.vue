    ...
    <template #header>
      <div class="flex items-center gap-2.5">
        ...
        <div>
          ...
          <p class="text-[12px] text-[#a7f3d0] font-mono m-0 opacity-90">{{ venta?.numeroFactura }}</p>
        </div>
      </div>
    </template>

    <div v-if="venta" class="flex flex-col gap-6 pt-2">
      <div class="bg-white rounded-xl border border-[#e2e8dd] p-4 shadow-sm flex flex-col gap-4">
        <div class="grid grid-cols-2 gap-4 pb-3 border-b border-[#e2e8dd]/60">
          <div>
            ...
            <p class="text-[14px] font-semibold text-[#1a2e1f] m-0">{{ venta.vendidoPor }}</p>
          </div>
          <div>
            <p class="text-[11px] uppercase tracking-wider text-[#6b7280] font-bold mb-1">Fecha de Emisión</p>
            <p class="text-[14px] font-semibold text-[#1a2e1f] m-0">{{ venta.fechaEmision }}</p>
          </div>
        </div>
        ...
        <div class="grid grid-cols-2 gap-4">
          <div>
            ...
            <p class="text-[14px] font-semibold text-[#1a2e1f] m-0">{{ venta.tipoPago }}</p>
          </div>
          <div>
            <p class="text-[11px] uppercase tracking-wider text-[#6b7280] font-bold mb-1">Estado Transacción</p>
            <div class="mt-0.5">
              <span :class="venta.status === 'Procesado'
      ...
      <div>
        ...
        <div class="rounded-xl border border-[#e2e8dd] overflow-hidden shadow-sm">
          <table class="w-full text-[13px]">
            ...
            <tbody>
              <tr
                v-for="(item, i) in venta.items"
                :key="i"
                :class="i % 2 === 0 ? 'bg-white' : 'bg-[#f9fafb]'"
                class="border-b last:border-0 border-[#e2e8dd] hover:bg-[#eef2e9] transition-colors"
              >
                <td class="px-4 py-2.5 text-[#1a2e1f] font-medium">{{ item.nombreProducto }}</td>
                <td class="px-4 py-2.5 text-center text-[#4b5563] font-semibold">{{ item.cantidad }}</td>
                <td class="px-4 py-2.5 text-right text-[#4b5563]">${{ formatCurrency(item.precio) }}</td>
                <td class="px-4 py-1.5 text-right text-[#4b5563] font-medium">{{ item.unidad }}</td>
                <td class="px-4 py-2.5 text-right font-bold text-[#1a2e1f]">${{ formatCurrency(item.cantidad * item.precio) }}</td>
              </tr>
            </tbody>
          </table>
        </div>
      ...
      <div class="flex justify-end">
        <div class="bg-[#f4f7f2] border border-[#e2e8dd] rounded-xl px-5 py-2.5 flex items-center gap-4 shadow-sm">
          ...
          <span class="text-[22px] font-black text-[#2b5e3b]">${{ formatCurrency(venta.total) }}</span>
        </div>
      </div>
    </div>
