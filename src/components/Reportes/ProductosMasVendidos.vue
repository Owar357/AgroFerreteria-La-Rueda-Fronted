  ...
  <div class="bg-[#eef2e9] min-h-screen p-6 font-['Inter',sans-serif]">
    ...
    <div class="flex items-center gap-3 mb-6">
      <div class="bg-white p-3 rounded-2xl shadow-sm border border-[#e2e8dd]">
        <i class="pi pi-star text-[24px] text-[#5F6B52]"></i>
      </div>
      <div>
        <h1 class="text-2xl font-bold text-[#1e3a2f]">Reporte de Productos Más Vendidos</h1>
        <p class="text-gray-500 text-sm mt-1">Ranking de los productos con mayor volumen de ventas.</p>
      </div>
    </div>
    ...
    <div class="bg-white rounded-2xl border border-[#e2e8dd] shadow-sm p-6">
      ...
      <div class="flex flex-wrap items-end gap-4">
        ...
        <div class="flex flex-col gap-1 w-[140px]">
          <label class="text-sm font-medium text-gray-600">Top (cant.)</label>
          <InputNumber
            v-model="limite"
            :min="1"
            placeholder="10"
...
const limite = ref(null) // si queda vacío, el backend usa su default (10)
...
const generarPDF = async () => {
  ...
  try {
    await generarReporteProductosMasVendidos({
