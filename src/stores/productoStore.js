...
export const useproductoStore = defineStore('productos', () => {
  ...
  const crearProducto = async (data) => {
    try {
      ...
      if (error.response?.status === 403) {
        await Swal.fire({ icon: 'error', title: 'Sin autorización', text: 'No tiene permisos.', confirmButtonColor: '#2b5e3b' })
        return { ok: false }
      }
      await Swal.fire({ icon: 'error', title: 'Error al crear el producto', text: error.response?.data?.message || 'Error del servidor.', confirmButtonColor: '#2b5e3b' })
      return { ok: false }
    }
  }
  ...
  return {
    ...
    //pra las acciones
    cargarProductos, cargarCategorias,  crearProducto
  }
})
