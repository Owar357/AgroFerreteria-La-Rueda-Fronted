            ...
            <StepPanel value="1" v-slot="{ activateCallback }">
              <div class="flex flex-col gap-4 pt-6">
                ...
                <div class="flex flex-col gap-1.5">
                  ...
                  <Select v-model="documentoForm.proveedor" :options="proveedoresOptions"
                    placeholder="Seleccionar proveedor" class="w-full bg-[#f9fafb] border-[#d1d5db] text-[#1a2e1f]" />
                </div>

                <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                  <div class="flex flex-col gap-1.5">
                    <label class="text-[14px] font-medium text-[#1a2e1f]">Tipo de comprobante</label>
                    <Select v-model="documentoForm.tipoComprobante" :options="comprobantesOptions"
                      placeholder="Seleccionar tipo" class="w-full bg-[#f9fafb] border-[#d1d5db]" />
                  </div>
                  <div class="flex flex-col gap-1.5 w-5">
                    <label class="text-[14px] font-medium text-[#1a2e1f]">Nº comprobante</label>
                    <InputText v-model="documentoForm.numComprobante" placeholder="DTE-03-000123456"
                ...
                <div class="grid grid-cols-1 md:grid-cols-3 gap-4">
                  ...
                  <div class="flex flex-col gap-1.5">
                    ...
                    <Select v-model="documentoForm.estadoPago" :options="estadosPagoOptions" placeholder="Seleccionar"
                      class="w-full bg-[#f9fafb] border-[#d1d5db]" />
                  </div>
                  <div class="flex flex-col gap-1.5">
                ...
                <div class="flex flex-col gap-4">
                  <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                    ...
                    <div class="flex flex-col w-4 gap-1.5">
                      <label class="text-[14px] font-medium text-[#1a2e1f]">Tipo de presentacion que entra</label>
                      <Select v-model="loteForm.tipoUnidad" :options="presentacionesLote" optionLabel="nombre"
                        placeholder="Seleccionar unidad" class="w-full bg-[#f9fafb] border-[#d1d5db]" />
                    </div>
                    <div class="flex flex-col gap-1.5">
                      <label class="text-[14px] font-medium text-[#1a2e1f]">Cantidad entrante</label>
                      <InputText v-model="loteForm.cantidadEntrante" placeholder="0"
...
const agregarItemsATabla = () => {
  if (!loteForm.producto || !loteForm.tipoUnidad) return

  console.log('cantidadUnidadEntrante:', loteForm.cantidadUnidadEntrante)
  console.log('loteForm completo:', JSON.stringify(loteForm))
  ...
  itemsAgregados.value.push({
    ...
    cantFact: loteForm.tipoUnidad.nombre,
    cantidad: loteForm.cantidadEntrante,
    costoUnit: `$${loteForm.costoUnitario || '0.00'}`,
    vencimiento: vencimientoTexto,
    subtotal: `$${(Number(loteForm.cantidadEntrante) * Number(loteForm.costoUnitario || 0)).toFixed(2)}`,
    presentacion_id: loteForm.tipoUnidad.id,
  })
