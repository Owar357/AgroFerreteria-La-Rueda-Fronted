      ...
      <div class="flex flex-col overflow-hidden" style="padding: 20px">
        ...
        <div class="flex flex-col gap-4 flex-1 overflow-y-auto">
          ...
          <div class="flex flex-col gap-1.5">
            ...
            <Select v-model="tipoFactura" :options="tiposFactura" optionLabel="label" optionValue="value" class="w-full" />
          </div>
            ...
            <DataTable :value="productosVenta" style="font-size: 12px" emptyMessage="No hay ningún producto registrado">
              ...
              <Column header="Cantidad">
                <template #body="slotProps">
                  <InputNumber v-model="slotProps.data.cantidad" :min="1" :maxFractionDigits="0"
                    showButtons buttonLayout="horizontal" inputStyle="width: 80px; text-align: center;" />
                </template>
              </Column>
              <Column header="Precio">
                <template #body="slotProps">${{ parseFloat(slotProps.data.precio).toFixed(2) }}</template>
              </Column>
              <Column header="Descuento">
                <template #body="slotProps">${{ parseFloat(slotProps.data.descuento).toFixed(2) }}</template>
              </Column>
              ...
              <Column header="" style="width: 40px">
                <template #body="slotProps">
                  <Button icon="pi pi-trash" severity="danger" text rounded size="small" @click="eliminarProducto(slotProps.index)" />
                </template>
              </Column>
            </DataTable>
          </div>
          ...
          <div v-if="tipoPago === 'efectivo'" class="flex flex-col gap-1.5">
            ...
            <div class="flex justify-between items-center gap-4">
              <div class="flex items-center gap-2 flex-1">
                ...
                <InputNumber v-model="efectivoRecibido" mode="currency" currency="USD" locale="en-US"
...
const subtotalGravado = computed(() =>
  productosVenta.value.reduce((acc, p) => p.aplica_iva ? acc + parseFloat((p.subtotal / 1.13).toFixed(4)) : acc, 0)
)
  ...
  try {
    ...
    if (error.response?.status === 404) {
      const resultado = await Swal.fire({
        icon: 'question', title: 'Cliente no encontrado',
        text: '¿Desea registrar un nuevo cliente?',
        showCancelButton: true, confirmButtonText: 'Registrar',
        cancelButtonText: 'Cancelar', confirmButtonColor: '#2b5e3b',
      })
